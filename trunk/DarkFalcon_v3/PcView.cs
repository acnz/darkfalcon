...
namespace DarkFalcon_v3
{
 ...
 graphics.PreferredBackBufferHeight, 0);
        ...
        public void NovoComponente(dfCom c)
        {
            switch(c.Tipo){
                case "Motherboard":
                    Mother = c;
                ...
                if (alpha > 0.6f)
                {
                    ...
                    foreach (_3DCheckbox cb in groupM)
                    {
                        ...
                        if (cb.IsChecked)
                        {
                            ...
                            {
                                case "Motherboard":
                                    ((_3DImageBox)listaHUD[listaHUD.Count - 1]).Image = Mother.Tipo + "//" + Mother.ID;
                                    break;
                                case "Processador":
                                    ((_3DImageBox)listaHUD[listaHUD.Count - 1]).Image = Proc.Tipo + "//" + Proc.ID;
                                    break;
                                case "Monitor":
                                    ((_3DImageBox)listaHUD[listaHUD.Count - 1]).Image = Monitor.Tipo + "//" + Monitor.ID;
                                    break;
                                case "Gabinete":
                                    ((_3DImageBox)listaHUD[listaHUD.Count - 1]).Image = Gabinete.Tipo + "//" + Gabinete.ID;
                                    break;
